...
export function parseConfig(args: string[], env: NodeJS.ProcessEnv): ProviderConfig {
  ...
  const known = new Set(['--ephemeral-wallet', '--alchemy-session', '--router', '--id', '--name', '--model', '--port', '--capacity', '--interval-ms', '--chunk-size', '--mode', '--input-price', '--cache-read-price', '--cache-write-price', '--output-price', '--min-reserve']);
  ...
  if (Number(ephemeral) + Number(alchemySession) + Number(Boolean(env.PROVIDER_PRIVATE_KEY)) > 1) throw new Error('钱包模式互斥：Alchemy session、临时钱包和 PROVIDER_PRIVATE_KEY 只能选择一种');
  return {
