...
## Native MON release: deployed contract, application switch pending

Current source now uses the independently deployed native market `0x142a4904307244Bed0cECD72dE8329A253333182` ([deployment and verification](../contracts/deployments/inferpool-mon-native-testnet.json)). Native/MON/18/router/runtime checks and both explorer verifications passed. The public server described above still runs the legacy dUSD release `319c6b9`; this contract deployment is not a public application switch.

Use `node scripts/deploy-mon-native.mjs` for read-only preflight, `--deploy` for explicit deployment/recovery, and `--verify-only` for existing-code/source verification. `node --import tsx scripts/native-monad.ts` is read-only by default; `--execute --smoke` performs the bounded same-wallet MON test documented in [the runbook](../docs/runbook.md#原生-mon-测试网当前源码). Current npm setup:monad and test:monad point to native-monad --execute and --execute --smoke; the old setup-monad.ts/smoke-monad.ts files remain historical dUSD tools. The new test:api:monad invokes smoke-native-api.ts --execute and has not run yet. Do not use historical scripts to initialize the new market.

Before switching, reconcile and stop the legacy writer, retain its private backup, and prepare a market-specific native ledger with the original orders/idempotency/createdAt preserved. Supply both `LEGACY_MARKET_ADDRESS=0x6F1b725DD3588cb5c8C3f72F614E80ebB2d82568` and `LEGACY_TOKEN_ADDRESS=0x62701D69bD213e8F63c28465528931de208cE06E`; set the new MARKET_ADDRESS and keep the same Router identity, admission epoch and limits. No old order, grant, quote or balance is converted to MON. Startup rejects unresolved legacy orders and unknown asset identities; an old POST key returns 409 with its old request ID rather than creating a MON charge.

Stage matching code, native ABI and the rebuilt Web together. Read `/config` back as new market/MON/18 plus the correct legacy fields, then reauthenticate seller A with its new-market quote. Native UI defaults are .1 MON deposit, .05 MON /24h grant and .001 MON request budget; they are not copies of old dUSD amounts. The smaller same-wallet contract smoke is .01/.005/.001 and has passed eight real testnet transactions: normal charge .000110 MON, seller failure zero, withdrawal .001, final escrow .009 and no locks ([evidence](../contracts/deployments/inferpool-native-monad-smoke.json)). This is separate from buyer Web approval and public HTTP/SSE acceptance. Source tests/builds do not establish new-origin wallet or streaming acceptance. Legacy Web controls remain available for old dUSD balances, withdrawals, revocation and reclaim using the old ABI.
...
The owner handles TLS and the existing proxy configuration. The application team provides the following values; the endpoint has passed page/API-read and seller WSS checks, while inference/SSE/settlement remain outside this deployment acceptance.
