...
async function main() {
  ...
  const app=createApp(engine,new Auth(store,domain),{trustProxy,webStaticDir,allowedOrigins:(process.env.ALLOWED_ORIGINS??'http://localhost:5173,http://localhost:3000').split(',').map(s=>s.trim()).filter(Boolean),publicConfig:{token_address:process.env.TOKEN_ADDRESS??process.env.DEMO_USD_ADDRESS,market_address:process.env.MARKET_ADDRESS,chain_id:chain.mode==='monad-testnet'?10143:31337}});
