...
import { parseTrustProxy } from './runtime-config.js';
import { resolveWebStaticDir } from './static-web.js';
async function main() {
  // Validate proxy and static-file configuration before wallet/RPC initialization or recovery.
  const trustProxy=parseTrustProxy(process.env.ROUTER_TRUST_PROXY);
