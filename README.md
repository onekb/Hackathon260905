...
业务合约已部署并验证源码，测试网 API、本地双卖家联调及独立买家的浏览器六种目标场景通过，含失败零收费、预算、缓存和主动取消；八笔实际请求已对账。网页 Key 生命周期已检查；Monad 双卖家、公网及浏览器钱包切换/断连幂等尚未验收。检查记录、范围和交易证据统一见 [开发进度](docs/progress.md)。
