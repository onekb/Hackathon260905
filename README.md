...
业务合约已部署并验证源码，浏览器六种目标场景、Monad 双卖家指定/自动匹配及网页手动选择均已对账，原失败与恢复留证。演示文案与素材已推送；公网目标 `demo.example.com` 的前端构建及远端 Node 环境已准备，应用端口与授权仍在部署，用户负责 HTTPS 代理。尚未公网验收，完整范围见 [开发进度](docs/progress.md)。
