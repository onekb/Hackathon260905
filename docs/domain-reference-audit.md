...
日期：2026-09-05（Asia/Shanghai）。先完成只读盘点，随后用户明确“只处理 git 仓库和 git 历史”。**GitHub `main` 与本地仓库历史已完成目标域名清理；两版 PPTX 及对应 PDF 已从当前跟踪和全部历史移除，仅保留为 ignored 本地文件，移除前后文件 SHA256 一致。** 最终重写版本为 `794139cd5d90a36c2e7c61e362c0afd433fbcf99`；完成回执文档已在 `4aadd18` 正常推送，以下核验数量以历史重写检查点为准。本页不重复旧域名字面量。
...
- 最终执行与验证结果见本页完成记录；范围依据见 [D18](requirements-and-decisions.md#d18--仅清理-git-仓库及历史中的演示域名)。
...
- 两版 PPTX 及对应 PDF 改为仅本地材料，已从当前跟踪和全部历史删除并加入 ignore。仓库文档不再提供这些文件的下载链接，历史生成与视觉检查仅代表当时产物，不代表当前用户编辑版本。最终新克隆已确认不含 PPT/PDF。
- 此前 `a4842c77b093d1930ef25f804cd8342c33d34873` 及 632 对象结果仅为中间阶段；下面记录再次重写及本地收尾的实际完成证据。

### 最终完成：远端与本地仓库一致

2026-09-05（Asia/Shanghai），以下对象与提交数量是最终重写 `794139cd5d90a36c2e7c61e362c0afd433fbcf99` 的核验快照，**不包含后续完成回执文档提交**。

- **第二次重写与推送：** 精确移除 4 个 PPTX/PDF 历史路径，逐一核对 23 个提交，只删除目标路径，其余文件、作者时间、消息及父关系均保持预期。相对第二阶段输入，19 个 SHA 不变、4 个变化。通过 SSH 精确 `force-with-lease` 将 GitHub `main` 从 `a4842c7` 更新至 `794139cd5d90a36c2e7c61e362c0afd433fbcf99`。
- **独立远端验证：** 再次从 GitHub 全新 mirror 克隆，得到 23 个 commit、171 个 tree、449 个 blob，共 643 对象；检查 1,442 个 tree 项和所有提交的 4,139 个文件路径。目标域名零命中，所有 `.ppt`/`.pptx`/`.pdf` 路径及对象均为 0，无不可达对象或 reflog。
- **原工作区收尾：** 根以 mixed 方式同步新历史，先为 `.git` 做本机私有完整备份，再移除 3 个旧 Codex snapshot refs 与 `ORIG_HEAD`、过期旧 reflog 并执行 GC。本地同样核对 643 个对象，目标域名、PPT/PDF 和不可达对象均为 0；`git fsck --full` 退出码为 0，已跟踪工作文件扫描零命中。
- **本地演示内容保护：** 4 个本地 PPTX/PDF 去跟踪前后 SHA256 完全相同，已被 ignore。用户编辑内容未因最终移除更改，当前版本未重新审阅；完成核验时 `git status` 干净。随后新增的完成回执文档是正常工作区修改，不改变这一核验事实。
- **证据连续性：** 两处历史源码 GitHub 链接已依据实际映射更新，服务器 release 标签保留。前序类型检查及 3 项离线 origin 拒绝用例继续作为本轮脚本修改证据；最终收尾未新增业务代码或交易，无需重复业务测试。

## 去除方案与边界

1. **当前公开文件：** 将部署样例改为示例域名，验收脚本改为配置参数；清理文档与提交材料；按用户最终要求，PPT/PDF 仅从 Git 跟踪与历史移除，不修改本地内容。可继续在私有运行配置中保留真实域名，保持现有演示可用。
2. **Git 提交历史：** 在隔离副本中重写包含引用的文件历史，并从全部历史移除目标 PPT/PDF；核对所有目标 refs 后才更新远端。单独新增删除提交不能清理旧版本。重写会改变首次相关提交及其后续提交的 SHA，需协调已有副本、旧 SHA 文档引用与部署版本对应关系。用户已明确授权此范围，执行完成与远端同步证据见本页最终完成记录。
...
未全面检查：工作区外副本、浏览器存储、Para 后台、1Panel 实际站点目录、域名解析/证书记录、服务器私有备份、已删除或不可访问的外部资料。上述未检查范围不属于本轮完成声明；可控 GitHub 主分支与本地仓库的最终清理证据已单列。仓库外本机私有备份不随此操作删除，GitHub 旧 SHA 缓存及第三方旧副本不保证清除；已有克隆应重新克隆，避免合回旧历史。
