...
**当前：业务合约已部署并完成源码验证；浏览器六种目标场景八单、Monad 双卖家 API 三场景与网页手动选择均完成对账。** 原锁款失败及一次受限恢复分别留证。五分钟文案与素材已推送，MOJO 单人 InferPool 队伍已建、项目未提交。公网目标为 `demo.example.com`，远端钱包授权与原账本迁移完成，Router 与卖家 A 已常驻运行，用户 HTTPS 代理下页面、健康检查和 WSS 认证通过；新域名钱包登录、SSE 推理与结算待验，B 当前离线。身份重合与剩余交互边界见 [开发进度与证据](progress.md)。
