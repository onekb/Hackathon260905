...
**当前：新原生 MON 合约已部署并验证 2/2，当前修复根 97/97、Web 13/13、类型/lint/构建通过；原生合约前序 41/41 与小额实链验收保留，公网已切到 0a82030；前序 fc646d8 网页单笔完成与结算已验，公网逐帧展示未采证。** https://demo.example.com 回读新市场 MON/18 与在线卖家 A 报价正常，公网 API/SSE 一单及浏览器签名登录/账户读取通过；独立买家 MON 网页存款/授权及正常、故障两单已验，逐帧 SSE 未采证；前序浏览器六目标场景、双卖家 API 三场景及网页手动单均按旧资产归档。MOJO [项目 #385](https://mojo.devnads.com/projects/385) 已提交、待审核，100 测试 MON 活动补给已核对到账；持续最新证据见 [开发进度](progress.md)。

**当前变更：** 用户已授权 D19 取消每日/累计请求次数与额外 Demo 并发限制并更新线上；保留预算、钱包授权和卖家容量。本轮 0a82030 已上线，源码/配置回读与服务器专项 18/18 通过，旧余量不再限制请求，完成证据见[进度](progress.md)。
