...
**18:35–18:37 只读就绪核对：核心演示与三页演讲已具备，剩五分钟计时彩排和同步备用 PDF。** 公网 health/config/models 200、23cc7f8 两服务 active、A normal/2 槽、8 单/0 未决；Chrome 正式演示 tab 的 B 已登录，Reasoner、.001预算/512输出/缓存关闭已准备。新回答与最近已确认账单可展示，本轮没有新增请求或交易；逐帧流式仍未采证。最新 WPS v2 比现有 PDF 新，需从最终 WPS 另导出备用 PDF，未代改演示文件，不建议继续加功能。
...
### 前序 fc646d8 流式修复：网页单笔结算已验、逐帧未采证（2026-09-05，Asia/Shanghai）
...
退役迁移脚本实际执行 exit 1，在状态访问前拒绝；部署环境过滤的本地纯函数检查确认只删三项旧键，回退保留原账本。实际复用 fc646d8 静态产物，index SHA256 `4c82e528c9ef56bd119aeaa5580055ce32414310ecac3b4359f027d254d74e4c`。代码已提交为 0a82030 并部署，双服务 active、5 单/0 未决/history 15；实际运行源码和环境已确认取消，未新增真实请求。

**实际发布证据（Asia/Shanghai）：** 18:00:57 开始受控发布，随后完成 Router/Provider 停启与版本切换；该起始时间对应私有备份 `/srv/inferpool/state/backups/unrestricted-20260905T100057Z`。原生 ledger 与备份逐字节一致，保留 5 单、凭证、幂等记录、history 15，零未决、权限 0600；仅从 router.env 删除三个退役 DEMO_* 键，其余原字节保留，provider.env 哈希一致。Linux npm ci 成功，staged request-guards + native-ledger **18/18** 通过。
...
### 演示与讲稿只读核对（2026-09-05，Asia/Shanghai）

18:35:18 公网 health/config/models 200，原市场 MON/18、A normal/2 槽；18:36:03 服务器 current 23cc7f8、两服务 active、8 单/0 未决，Alchemy valid/active 至 9 月 12 日 14:33:39。18:35:59 固定块 **59882957** 核对 B 托管 **.0990148 MON**、剩余授权 **.0491806 MON**，有效至 **2026-09-06 17:22:16（+08）**；B 钱包 Gas **.785624238 MON**、Router Gas **100.250057574 MON**，totalLocked 0、quote active v1，可覆盖 .001 MON 请求预算。

Chrome 正式 tab 已登录，最近用户单 [7d9a66ea-2fca-4ba8-bfee-1531622bfe7c](https://testnet.monadscan.com/tx/0x52b4ba048822686c4dfb958256c59989b9304c76aa9452b799bebb6a01ad774b) 展示新回答“AI推理演示”和链上已确认账单：输入54/输出181，费 **.000161 MON**、释放 **.000839 MON**。18:37:15 独立 RPC 核对既有 `215a9738…` 与 `7d9a66ea…` 的结算回执 success，getOrder state2/outcome0、预算 .001、费用及输出一致；创建至更新时间约 **21.87/22.879 秒**，只是历史观察，不保证现场时延。本轮只读，没有新下单或交易，未捕捉逐帧输出。

当前 WPS v2 为3页供需/Monad双向价值/流程，备注576非空字符，每页约40秒讲重点可安排2分钟；没有宣称已接真实AI或低价已被证明。只做内容/XML与画布边界检查，未做当前WPS全屏渲染验收。PPT保存于17:18:46，PDF为17:14:39前稿，后续标题、署名与页底等修改未同步；需由最终WPS另导出备用PDF，本轮未覆盖文件。演示采用2分钟PPT加3分钟网页，一新正常单与历史故障单；口头交代固定演示响应、真实Monad测试网资金结算及真实模型为下一步即可。
...
优先使用本轮正常账单与两单对照截图；演示前重读余额、授权有效期与卖家空闲容量，保持卖家 normal。前次修复验收时全局剩 4 次、B 当日剩 1 次仅为旧策略快照；D19 已完成回归并在 0a82030 上线，取消计数与额外 Demo 并发门槛，不再要求保留固定请求次数。浏览器逐帧 SSE 未采证，前序 API 的 34 批增量单独说明。
