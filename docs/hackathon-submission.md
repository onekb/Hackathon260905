...
| 持续可用 | 有条件具备，需运行维护与续批 | 两服务 enabled/active；用户已通过 D19 要求取消每日/累计次数与额外 Demo 并发门槛，代码与本地回归已完成、待发布，准确上线状态见[进度](progress.md)。此前 6/10 次为工程保护，非赛事要求，相关余量仅历史；资金授权、预算和卖家容量继续适用。Linux 会话到期 2026-09-12 14:33 Asia/Shanghai，持续签名需续批；官方未给具体长期天数 |
