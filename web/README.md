...
InferPool 的买家与卖家操作界面，使用 Next.js 16.3.4、React、Para Lite 邮箱内嵌 EVM 钱包和 viem。所有推理输出、Token 和缓存都是模拟；资金使用 Monad 测试网的自建 DemoUSD。生产构建、lint、冷刷新市场和邮箱登录弹窗已通过检查，邮箱登录及完整钱包交易仍待验证，最新证据见 [开发进度](../docs/progress.md)。
...
2. 为**这个新钱包地址**准备 Monad 测试网 MON。可把完整地址填入 [官方水龙头](https://faucet.monad.xyz/)；顶部含 `…` 的缩写不能用作收款地址。新钱包不继承已有 Alchemy 演示钱包的余额、Gas 或授权。
