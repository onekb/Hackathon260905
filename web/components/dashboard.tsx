...
const money = (s: string | number = 0) => Number(s).toLocaleString('en-US', { maximumFractionDigits: 6 });
...
export default function Dashboard({ wallet, config }: { wallet: WalletAccess; config: MarketConfig }) {
  ...
  const identity = `${config.chain_id}:${config.market_address.toLowerCase()}:${config.token_address.toLowerCase()}:${wallet.address?.toLowerCase() ?? 'disconnected'}`;
...
function DashboardSession({ wallet, config, tab, setTab }: { wallet: WalletAccess; config: MarketConfig; tab: string; setTab: (tab: string) => void }) {
  ...
  async function run() {
    ...
    if (!attempt || attempt.ended) {
      ...
      if (!/^(0|[1-9]\d*)(\.\d{1,6})?$/.test(budget) || Number(budget) <= 0) return setError('预算必须是大于零、最多 6 位小数的金额。');
...
function Bill({order:o,config,wallet,onRefresh}:{order:Order|null;config:MarketConfig;wallet:WalletAccess;onRefresh:()=>Promise<void>}) {
  ...
  async function reclaim() {
    if (!o || inFlight.current || reclaimed || o.buyer.toLowerCase()!==wallet.address?.toLowerCase()) return;
    inFlight.current=true; setWorking(true); setStatus('');
    try {
      await wallet.sendContract('market','reclaimExpired',[keccak256(stringToHex(o.id))]);
      if (!alive.current) return;
  ...
  return <aside className="panel bill"><div className="panel-title"><h2>本次账单</h2><span className={`subtle-pill ${o?.billConfirmed?'good':''}`}>{o?.billConfirmed?'链上已确认':'预计 / 待确认'}</span></div><div className="bill-total"><span>{o?.billConfirmed?'实际费用':'结算后显示实际费用'}</span><strong>{o?.billConfirmed?money(o.charge):'—'}<small>dUSD</small></strong></div><div className="bill-line"><span>预算预留</span><b>{o?money(o.budget):'—'}</b></div><div className="bill-line"><span>{o?.billConfirmed?'已释放预算':'待释放预算'}</span><b className="green">{o?money(o.released):'—'}</b></div><div className="bill-breakdown"><div className="breakdown-heading"><span>计量明细</span><span>模拟 Token</span></div>{(Object.keys(priceLabels) as PriceKey[]).map(k=><div className="bill-line" key={k}><span>{k==='output'?<ArrowUpRight size={14}/>:<ArrowDownLeft size={14}/>} {priceLabels[k]}</span><b>{o?o.usage[k].toLocaleString():'—'}</b></div>)}</div><div className="bill-info"><ShieldCheck size={18}/><p>只收实际用量费用。<br/>卖家故障时，整单推理费为零。</p></div>{o&&<div className="bill-meta"><p>请求 <code>{o.id}</code></p><p>卖家 <code>{short(o.seller)}</code></p><p>报价版本 {o.quote.version||'—'} · 缓存 {o.cacheMode}</p><p>{statuses[o.status]}</p>{o.settlementError&&<p className="error">结算待处理：{o.settlementError}</p>}{link&&<a className="text-button green" target="_blank" rel="noreferrer" href={link}>查看结算交易 <ArrowUpRight size={15}/></a>}{o.lockTx&&txUrl(o.lockTx,config.chain_id)&&<a className="text-button" target="_blank" rel="noreferrer" href={txUrl(o.lockTx,config.chain_id)}>查看锁款交易 <ArrowUpRight size={15}/></a>}{!o.billConfirmed&&o.lockTx&&o.status!=='lock_failed'&&!reclaimed&&expired&&<button className="button secondary" disabled={working} onClick={reclaim}>直接取回超时锁款</button>}{status&&<p role="status">{status}</p>}</div>}</aside>;
}
