...
const configuredOrigin=process.env.INFERPOOL_API_ORIGIN?.trim();
assert.ok(configuredOrigin,'Set INFERPOOL_API_ORIGIN to the Router origin before running this script');
const originUrl=new URL(configuredOrigin);
assert.ok(['http:','https:'].includes(originUrl.protocol),'INFERPOOL_API_ORIGIN must use HTTP or HTTPS');
assert.ok(!originUrl.username&&!originUrl.password&&!originUrl.search&&!originUrl.hash&&originUrl.pathname==='/','INFERPOOL_API_ORIGIN must be a bare origin without credentials, path, query, or fragment');
