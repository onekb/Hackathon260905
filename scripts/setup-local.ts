...
const config = { chainMode: 'anvil', chainId: 31337, rpcUrl, asset: { native: true, symbol: 'MON', decimals: 18 }, market: f.market, router: f.router, buyer: f.buyer, sellerA: f.sellerA, sellerB: f.sellerB, model: f.model, mockInference: true };
writeFileSync('.local/deployment.json', JSON.stringify(config, null, 2) + '\n', { mode: 0o600 });
writeFileSync('.local/router.env', `CHAIN_MODE=anvil\nRPC_URL=${rpcUrl}\nMARKET_ADDRESS=${f.market}\nROUTER_ADDRESS=${f.router}\nROUTER_PUBLIC_URL=http://127.0.0.1:8787\nROUTER_STATE_PATH=.local/router-state-native-${f.market.toLowerCase()}.json\n`, { mode: 0o600 });
